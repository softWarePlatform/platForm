...
const NAV_ITEMS: NavItem[] = [
  { to: "/", label: "主界面", end: true, roles: ["STUDENT", "TEACHER"] },
  { to: "/enrollment", label: "选课系统", roles: ["STUDENT"] },
  { to: "/teaching", label: "教学台", roles: ["TEACHER"] },
  { to: "/my-homework", label: "我的作业", roles: ["STUDENT"] },
  { to: "/my-labs", label: "我的实验", roles: ["STUDENT"] },
  { to: "/teaching/homework", label: "作业批改", roles: ["TEACHER"] },
  { to: "/teaching/labs", label: "实验管理", roles: ["TEACHER"] },
  { to: "/admin", label: "管理控制台", end: true, roles: ["ADMIN"] },
              ...
              {user ? (
                <>
                  <span className="muted app-header__user">
                    {user.name}（
                    {user.role === "TEACHER" ? "教师" : user.role === "ADMIN" ? "超级管理员" : "学生"}）
                  </span>
