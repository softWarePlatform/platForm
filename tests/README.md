...
| 层级 | 目录 | 当前入口 | 依赖环境 |
| --- | --- | --- | --- |
| Unit | `backend/tests/unit/` | `npm run test:unit` | Node.js，无数据库；43 条 |
...
| E2E | `tests/e2e/` | 后续按计划补充 | 前端、API、Worker、数据库、Redis |

根目录 `npm test` 依次执行 Unit、DAO 和 API 测试，任一层失败都会返回非零退出码。API 报告输出到 `test-results/api-use-cases.json`。
