...
const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/admin/users/:userId/logs", { preHandler: authRequired("ADMIN") }, async (req, reply) => {
    ...
    const [enrollmentLogs, authoredAnnouncements, createdCourses, notifications] = await Promise.all([
    ...
    return {
      ...
      logs: [
        ...enrollmentLogs.map((log) => ({
          id: `enrollment-${log.id}`,
          type: "enrollment",
          title: `${log.course.title} · ${log.action}`,
          detail: log.note ?? `操作人：${log.operator?.name ?? "系统"}`,
        ...
        ...createdCourses.map((course) => ({
          ...
          detail: course.published ? "已发布" : "未发布",
          createdAt: course.createdAt,
        })),
        ...authoredAnnouncements.map((item) => ({
          id: `announcement-${item.id}`,
          type: "announcement",
          title: `发布公告：${item.title}`,
        ...
        ...notifications.map((item) => ({
          ...
          type: "notification",
          title: item.title,
          detail: `${item.type}${item.readAt ? " · 已读" : " · 未读"}`,
          createdAt: item.createdAt,
        })),
  ...
  app.delete("/admin/users/:userId", { preHandler: authRequired("ADMIN") }, async (req, reply) => {
    const params = z.object({ userId: z.string().min(1) }).safeParse(req.params);
    if (!params.success) return reply.code(400).send({ error: "参数无效" });
    if (req.auth!.sub === params.data.userId) return reply.code(400).send({ error: "不能删除当前登录账号" });
