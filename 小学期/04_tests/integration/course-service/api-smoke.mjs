...
assert.ok(Array.isArray(access.body.access.classIds));
const roster = await call(`/internal/courses/${primary.id}/enrollments?page=1&pageSize=200`, { headers: internal() });
assert.equal(roster.status, 200);
assert.equal(roster.body.courseId, primary.id);
assert.equal(roster.body.page, 1);
assert.ok(roster.body.items.some((row) => row.id === studentMe.body.user.id));
...
const missingKey = await call("/internal/notifications", { method: "POST", headers: { ...internal(), "content-type": "application/json" }, body: JSON.stringify({ userIds: [studentMe.body.user.id], title: "缺少幂等键" }) });
assert.equal(missingKey.status, 400);
const notificationPayload = JSON.stringify({ userIds: [studentMe.body.user.id], type: "HOMEWORK", title: "内部通知冒烟", body: "来自服务调用", linkPath: "/homework/smoke" });
...
assert.equal(notification.body.created, 1);
