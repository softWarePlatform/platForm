...
await request("POST", `/enrollment/courses/${courseId}/enroll`, { token: studentAuth.token, body: {} });
...
const config = {
  ...
  sameMachineEvidence: "Windows Docker Desktop Kubernetes port-forward 2026-09-03",
  runs: 3,
  concurrency: 10,
  requestsPerTarget: 30,
  requestTimeoutMs: 10_000,
  targets: [
    { name: "dashboard", path: "/dashboard/me", headers: { authorization: `Bearer ${studentAuth.token}` } },
    { name: "homework-mine", path: "/homework/mine", headers: { authorization: `Bearer ${studentAuth.token}` } },
    { name: "lab-overview", path: "/lab-sets/mine/overview", headers: { authorization: `Bearer ${studentAuth.token}` } },
  ],
};
const manifest = {
  ...
  resources: { courseId, homeworkId: homework.homework?.id ?? null, labSetId, labId: lab.lab?.id ?? null },
