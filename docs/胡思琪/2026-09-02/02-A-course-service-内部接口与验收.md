...
| 能力 | 路径 | 调用方可用字段 | 失败语义 |
| --- | --- | --- | --- |
| 用户与角色 | `GET /internal/users/:userId` | `id,email,name,role` | `404 USER_NOT_FOUND` |
| 课程 | `GET /internal/courses/:courseId` | 课程、教师、容量、已选数 | `404 COURSE_NOT_FOUND` |
| 课程权限 | `GET /internal/courses/:courseId/access/:userId` | 服务端查询的 `role,canView,isTeacher,isEnrolled,classId` | 404；不接受调用方传入 role |
| 学生名单 | `GET /internal/courses/:courseId/enrollments` | `user` 与 `classId` | 404 |
| 班级 | `GET /internal/courses/:courseId/classes` | 班级、选课人数 | 404 |
| 通知写入 | `POST /internal/notifications` | 通知实体、`idempotentReplay` | 缺少合法 `Idempotency-Key` 为 `400` |
...
课程服务向 B/C 服务的地址分别从 `HOMEWORK_SERVICE_URL`、`LAB_SERVICE_URL` 读取，调用：

```text
POST {SERVICE_URL}/internal/dashboard/course-summaries:batch
Headers: x-internal-service-token, x-request-id
Body: { "userId": "UUID", "courseIds": ["UUID"] }
```

期望响应为 `{ summaries: [{ courseId, pendingCount?, progressPercent?, deadlines? }] }`。上游未配置、超时或非 2xx 时，`GET /dashboard/me` 返回 `dependencies.{homework|lab}.status = "UNAVAILABLE"` 与 `reason`；对应课程的 `homework` / `lab` 为 `null`。
