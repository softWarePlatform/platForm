...
import { fetchCourseAccess, fetchCourseInfo, fetchCourseRoster } from "../course-client.js";
...
async function candidateCourseIds(courseIdFilter?: string): Promise<string[]> {
  if (courseIdFilter) return [courseIdFilter];
  const rows = await prisma.labSet.findMany({ distinct: ["courseId"], select: { courseId: true } });
  return rows.map((row) => row.courseId);
}

async function loadLabSetsByCourse(courseIds: string[]): Promise<Map<string, LabSetRow[]>> {
  const result = new Map<string, LabSetRow[]>();
  if (!courseIds.length) return result;
  const rows = await prisma.labSet.findMany({
    where: { courseId: { in: courseIds } },
    ...labSetOverviewInclude,
  });
  for (const row of rows as LabSetRow[]) {
...
async function loadCoursesForStudent(userId: string, courseIdFilter?: string, requestId?: string): Promise<CourseBundle[]> {
  const ids = await candidateCourseIds(courseIdFilter);
  const allowed = await Promise.all(ids.map(async (id) => {
    const access = await fetchCourseAccess(id, userId, requestId);
    return access?.canView ? fetchCourseInfo(id, requestId) : null;
  }));
  const courses = allowed.filter((course): course is NonNullable<typeof course> => Boolean(course));
  const sets = await loadLabSetsByCourse(courses.map((course) => course.id));
  return courses.map((course) => ({ id: course.id, title: course.title, labSets: sets.get(course.id) ?? [] }));
}
...
async function loadCoursesForTeacher(
  ...
  const ids = await candidateCourseIds(courseIdFilter);
  const loaded = await Promise.all(ids.map((id) => fetchCourseInfo(id, requestId)));
  const courses = loaded.filter((course): course is NonNullable<typeof course> =>
    Boolean(course && (role === "ADMIN" || course.teacherId === userId)),
  );
  const sets = await loadLabSetsByCourse(courses.map((course) => course.id));
  return courses.map((course) => ({ id: course.id, title: course.title, labSets: sets.get(course.id) ?? [] }));
}
...
async function loadEnrollmentCounts(courseIds: string[], requestId?: string): Promise<Map<string, string[]>> {
  const map = new Map<string, string[]>();
  if (courseIds.length === 0) return map;
  for (const courseId of courseIds) {
    const users = await fetchCourseRoster(courseId, requestId);
    map.set(courseId, users.map((user) => user.id));
  }
  return map;
}
...
export async function buildStudentLabSetOverview(opts: {
  ...
  const courses = await loadCoursesForStudent(opts.userId, opts.courseId, opts.requestId);
...
export async function buildTeacherLabSetOverview(opts: {
  ...
  const nowMs = opts.nowMs ?? Date.now();
  const courses = await loadCoursesForTeacher(opts.userId, opts.role, opts.courseId, opts.requestId);
  const courseIds = courses.map((c) => c.id);
  const enrollmentByCourse = await loadEnrollmentCounts(courseIds, opts.requestId);
  const allLabIds = courses.flatMap((c) => c.labSets.flatMap((s) => s.labs.map((l) => l.id)));
  const submissions = await loadSubmissionsForLabIds(allLabIds);
