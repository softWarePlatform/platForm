...
const labSetsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const ok = await canAccessCourse(req.auth!.sub, courseId, req.id);
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const enrolledIds = (await fetchCourseRoster(courseId, req.id)).map((student) => student.id);
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const enrollments = await fetchCourseRoster(courseId, req.id);
