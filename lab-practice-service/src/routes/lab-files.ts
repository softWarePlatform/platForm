...
import { fetchCourseAccess, fetchCourseInfo, fetchCourseUsers } from "../course-client.js";

async function canAccessLab(userId: string, labId: string, requestId: string) {
  const lab = await prisma.lab.findUnique({
    where: { id: labId },
  });
  if (!lab) return { ok: false as const, reason: "实验不存在" };
  const [course, courseAccess] = await Promise.all([
    fetchCourseInfo(lab.courseId, requestId),
    fetchCourseAccess(lab.courseId, userId, requestId),
  ]);
  if (!course || !courseAccess?.canView) return { ok: false as const, reason: "未选课" };
  return { ok: true as const, lab, course, courseAccess };
}
...
const labFilesRoutes: FastifyPluginAsync = async (app) => {
  app.get("/labs/:labId/files", { preHandler: authRequired() }, async (req, reply) => {
    ...
    const check = await canAccessLab(req.auth!.sub, labId, req.id);
    ...
    const users = await fetchCourseUsers(rows.map((row) => row.uploadedById), req.id);
    const byId = new Map(users.users.map((user) => [user.id, user]));
    return { files: rows.map((row) => ({ ...row, uploadedBy: byId.get(row.uploadedById) ?? null })) };
  });
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const check = await canAccessLab(req.auth!.sub, labId, req.id);
      if (!check.ok) return reply.code(404).send({ error: check.reason });
      if (req.auth!.role !== "ADMIN" && check.course.teacherId !== req.auth!.sub) {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const row = await prisma.labFile.findFirst({
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (row) {
        ...
        await prisma.labFile.delete({ where: { id: row.id } });
      }
