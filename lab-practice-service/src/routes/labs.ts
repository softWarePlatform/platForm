...
import { fetchCourseAccess, fetchCourseInfo, fetchCourseRoster, fetchCourseUsers } from "../course-client.js";
...
async function canAccessCourse(
  ...
  return Boolean((await fetchCourseAccess(courseId, userId, requestId))?.canView);
}
...
const labsRoutes: FastifyPluginAsync = async (app) => {
  app.get(
    ...
    async (req, reply) => {
      ...
      const course = await fetchCourseInfo(courseId, req.id);
      if (!course) return reply.code(404).send({ error: "课程不存在" });

      const ok = await canAccessCourse(req.auth!.sub, req.auth!.role, courseId, course.teacherId, req.id);
  ...
  app.get("/labs/:id", { preHandler: authRequired() }, async (req, reply) => {
    ...
    const course = await fetchCourseInfo(lab.courseId, req.id);
    if (!course) return reply.code(404).send({ error: "课程不存在" });
    ...
    const privileged = isCoursePrivileged(req.auth!.role, course.teacherId, req.auth!.sub);
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const enrollRows = await fetchCourseRoster(lab.courseId, req.id);
      const enrolledIds = new Set(enrollRows.map((e) => e.id));
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      const course = lab ? await fetchCourseInfo(lab.courseId, req.id) : null;
      if (!lab || !course || (req.auth!.role !== "ADMIN" && course.teacherId !== req.auth!.sub)) {
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const lab = await prisma.lab.findUnique({ where: { id: tc.labId } });
      const course = lab ? await fetchCourseInfo(lab.courseId, req.id) : null;
      if (!lab || !course || (req.auth!.role !== "ADMIN" && course.teacherId !== req.auth!.sub)) {
        return reply.code(403).send({ error: "无权编辑" });
      }
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const course = await fetchCourseInfo(sub.lab.courseId, req.id);
      if (!course || (req.auth!.role !== "ADMIN" && course.teacherId !== req.auth!.sub)) {
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      const sub = await prisma.submission.findUnique({
        where: { id: submissionId },
        include: { lab: { include: { labSet: true } } },
      });
      if (!sub) return reply.code(404).send({ error: "记录不存在" });
      const course = await fetchCourseInfo(sub.lab.courseId, req.id);
      if (!course || (req.auth!.role !== "ADMIN" && course.teacherId !== req.auth!.sub)) {
  ...
  app.get("/labs/:id/submissions", { preHandler: authRequired() }, async (req, reply) => {
    ...
    const course = await fetchCourseInfo(lab.courseId, req.id);
    if (!course || (course.teacherId !== req.auth!.sub && req.auth!.role !== "ADMIN")) {
    ...
    const users = await fetchCourseUsers(rows.map((row) => row.userId), req.id);
    const byId = new Map(users.users.map((user) => [user.id, user]));
    return { submissions: rows.map((row) => ({ ...row, user: byId.get(row.userId) ?? null })) };
  });
  ...
  app.get("/submissions/:submissionId", { preHandler: authRequired() }, async (req, reply) => {
    ...
    if (req.auth!.role === "STUDENT") {
      ...
      const course = await fetchCourseInfo(sub.lab.courseId, req.id);
      if (!course || course.teacherId !== req.auth!.sub) return reply.code(403).send({ error: "无权查看" });
    }

    const users = await fetchCourseUsers([sub.userId], req.id);
    return { submission: { ...sub, user: users.users[0] ?? null } };
  });
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const isOwner = sub.userId === req.auth!.sub;
      const course = await fetchCourseInfo(sub.lab.courseId, req.id);
      const isTeacher = req.auth!.role !== "STUDENT" && (req.auth!.role === "ADMIN" || course?.teacherId === req.auth!.sub);
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      const course = await fetchCourseInfo(base.lab.courseId, req.id);
      if (!course || (req.auth!.role !== "ADMIN" && course.teacherId !== req.auth!.sub)) {
        return reply.code(403).send({ error: "无权查看" });
      }
      ...
      const userResult = await fetchCourseUsers([base.userId, ...others.map((row) => row.userId)], req.id);
      ...
      const scored = others.map((o) => {
        ...
        return {
          ...
          user: usersById.get(o.userId) ?? null,
        };
      });
      ...
      return {
        base: { submissionId: base.id, user: usersById.get(base.userId) ?? null },
